...
def bragg_integrado_com_phase_shifter(emeApi,Lambda,W_great,W_small,width,thickness,w_box,t_box,material_Si,material_BOX,wvg_start,wvg_stop,nb,
                                        Temperatura=300,periods=150, comprimento_de_onda = 1500e-9,mesh_multiplier=5, Phases =1, condicao="PML"):
...
def funcao_que_gera_o_ambiente_de_simulacao_e_calcula_a_potencia_na_saida_de_n_laser_mzi(inter,guia_menor, delta_L,x_1,defasagem,diretorio_dos_parametros_S):
        # funcao cria ambiente de simulacao no INTERCONNECT composto por dois laser, um combiner, um dispositivo arbitrario (a depender do parametro S),
        #  um splitter, dois filtros, dois fotodectores e dois medidores de potencia. Posteriormente simula e retorna os valores de potencia lidos nos medidores de potencia.
        ...
        inter.setposition("photodec1", 1200,0)
        inter.setposition("photodec2", 1200,200)

        inter.setposition("PM1", 1400,0)
        inter.setposition("PM2", 1400,200)
        ...
        inter.connect("spars1","port 2", "Y1", "opt_a1")
        ...
        inter.connect("Y2","opt_2", "photodec1", "input")
        inter.connect("Y2","opt_1", "photodec2", "input")
